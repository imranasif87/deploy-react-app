...
export const handleDownloadReportCrew = async (
  crewId: number,
  crewList: Array<any>
) => {
  ...
  const soul = crewList.find((soul: SoulData) => soul.id === crewId);
  const dataSoul = [
    [
      soul.id.toString(),
  ...
  // Add the heading
  doc.text("Pårørende", 10, doc.autoTable.previous.finalY + 15);
  ...
  // Add the heading
  doc.text("Hotell", 10, doc.autoTable.previous.finalY + 15);
  ...
  doc.text("Transport", 10, doc.autoTable.previous.finalY + 15);
  ...
  // Add heading
  doc.text("Begravelse", 10, doc.autoTable.previous.finalY + 15);
